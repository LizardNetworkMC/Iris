...
public class CommandIris implements DecreeExecutor {
    ...
    public void create(
        ...
        try {
            ...
            if (main) {
                Runtime.getRuntime().addShutdownHook(mainWorld.updateAndGet(old -> {
                    if (old != null) {
                        Runtime.getRuntime().removeShutdownHook(old);
                    }
        ...
        if (main) {
            sender().sendMessage(C.GREEN + "Your world will automatically be set as the main world when the server restarts.");
        }
    }
    ...
    public void download(
        ...
        if (pack.equalsIgnoreCase(GitHub.getDimensionPackName())) {
