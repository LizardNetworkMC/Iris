...
public class IrisDimension extends IrisRegistrant {
    ...
    public void installBiomes(IDataFixer fixer, DataProvider data, KList<File> folders, KSet<String> biomes) {
        ...
        File pack = data.getData().getDataFolder();
        File loot = new File(pack, "loot");
        if (!loot.isDirectory()) {
            return;
        }
        ...
        for (File datapacks : folders) {
            String lootTableChestDir = Paths.get(datapacks.getPath(), "iris", "data", getLoadKey().toLowerCase(), datapackLootTableDirName, "chests").toString();
            try {
